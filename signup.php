...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    $check_query = $conn->prepare("SELECT * FROM INVENTORY.USER WHERE email = ?");
    ...
    if ($check_result->num_rows > 0) {
        ...
        $insert_query = $conn->prepare("INSERT INTO INVENTORY.USER (user, email, pass) VALUES (?, ?, ?)");
        $insert_query->bind_param("sss", $username, $email, $password);

        if ($insert_query->execute()) {
            // Create a User object after successful signup
            $user = new User($conn->insert_id, $username, $email, $password);
